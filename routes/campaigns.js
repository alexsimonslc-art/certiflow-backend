...
// Initialize Supabase (Ensure these match your actual env variable names)
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const googleToken = req.user.googleToken; 

    let backupSheetLink = null;

    // 1. Generate Google Sheet if backup_data exists
    if (backup_data && backup_data.length > 0 && googleToken) {
